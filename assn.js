function add_row(){
	...
	for(var i=0;i<1000;i++){
		if(!localStorage['row'+i]){
			...
			localStorage['row'+table_len]=JSON.stringify(name);
			break;
...
window.onload=function(){
	...
	for(var i=0;i<1000;i++){
		if(localStorage['row'+i]){
			...
			var name=JSON.parse(localStorage['row'+i]);
			table_len=i;
			//window.console.log(row.getElementsByTagName("td")[0].innerHTML);
			table.insertRow(j).outerHTML="<tr id='row"+table_len+"'><td id='name_row"+table_len+"'>"+name+"</td><td><input type='button' id='edit_button"+table_len+"' value='Edit' class='edit' onclick='edit_row("+table_len+")'> <input type='button' value='Delete' class='delete' onclick='delete_row("+table_len+")'></td></tr>";
			j++;
		}
}
}